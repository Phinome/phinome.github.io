<h1 id="minimatch">minimatch</h1><p>A minimal matching utility.</p>
...
<h2 id="Usage">Usage</h2><pre><code class="javascript"><span class="keyword">var</span> minimatch = <span class="built_in">require</span>(<span class="string">"minimatch"</span>)
...
<h2 id="Features">Features</h2><p>Supports these glob features:</p>
<ul>
...
<h2 id="Minimatch_Class">Minimatch Class</h2><p>Create a minimatch object by instanting the <code>minimatch.Minimatch</code> class.</p>
...
<h2 id="Functions">Functions</h2><p>The top-level exported function has a <code>cache</code> property, which is an LRU<br>cache set to store 100 items.  So, calling these methods repeatedly<br>with the same pattern and options will use the same Minimatch object,<br>saving the cost of parsing it multiple times.</p>
<h3 id="minimatch(path,_pattern,_options)">minimatch(path, pattern, options)</h3><p>Main export.  Tests a path against the pattern using the options.</p>
...
<h3 id="minimatch-filter(pattern,_options)">minimatch.filter(pattern, options)</h3><p>Returns a function that tests its<br>supplied argument, suitable for use with <code>Array.filter</code>.  Example:</p>
...
<h3 id="minimatch-match(list,_pattern,_options)">minimatch.match(list, pattern, options)</h3><p>Match against the list of<br>files, in the style of fnmatch or glob.  If nothing is matched, and<br>options.nonull is set, then return a list containing the pattern itself.</p>
...
<h3 id="minimatch-makeRe(pattern,_options)">minimatch.makeRe(pattern, options)</h3><p>Make a regular expression object from the pattern.</p>
<h2 id="Options">Options</h2><p>All options are <code>false</code> by default.</p>
<h3 id="debug">debug</h3><p>Dump a ton of stuff to stderr.</p>
<h3 id="nobrace">nobrace</h3><p>Do not expand <code>{a,b}</code> and <code>{1..3}</code> brace sets.</p>
<h3 id="noglobstar">noglobstar</h3><p>Disable <code>**</code> matching against multiple folder names.</p>
<h3 id="dot">dot</h3><p>Allow patterns to match filenames starting with a period, even if<br>the pattern does not explicitly have a period in that spot.</p>
<p>Note that by default, <code>a/**/b</code> will <strong>not</strong> match <code>a/.d/b</code>, unless <code>dot</code><br>is set.</p>
<h3 id="noext">noext</h3><p>Disable “extglob” style patterns like <code>+(a|b)</code>.</p>
<h3 id="nocase">nocase</h3><p>Perform a case-insensitive match.</p>
<h3 id="nonull">nonull</h3><p>When a match is not found by <code>minimatch.match</code>, return a list containing<br>the pattern itself if this option is set.  When not set, an empty list<br>is returned if there are no matches.</p>
<h3 id="matchBase">matchBase</h3><p>If set, then patterns without slashes will be matched<br>against the basename of the path if it contains slashes.  For example,<br><code>a?b</code> would match the path <code>/xyz/123/acb</code>, but not <code>/xyz/acb/123</code>.</p>
<h3 id="nocomment">nocomment</h3><p>Suppress the behavior of treating <code>#</code> at the start of a pattern as a<br>comment.</p>
<h3 id="nonegate">nonegate</h3><p>Suppress the behavior of treating a leading <code>!</code> character as negation.</p>
<h3 id="flipNegate">flipNegate</h3><p>Returns from negate expressions the same as if they were not negated.<br>(Ie, true on a hit, false on a miss.)</p>
<h2 id="Comparisons_to_other_fnmatch/glob_implementations">Comparisons to other fnmatch/glob implementations</h2><p>While strict compliance with the existing standards is a worthwhile<br>goal, some discrepancies exist between minimatch and other<br>implementations, and are intentional.</p>
