<h1 id="flagged-respawn">flagged-respawn <a href="http://travis-ci.org/tkellen/node-flagged-respawn"><img src="https://secure.travis-ci.org/tkellen/node-flagged-respawn.png" alt="Build Status"></a></h1><blockquote>
...
<h2 id="What_is_it?">What is it?</h2><p>Say you wrote a command line tool that runs arbitrary javascript (e.g. task runner, test framework, etc). For the sake of discussion, let’s pretend it’s a testing harness you’ve named <code>testify</code>.</p>
...
<h2 id="Sample_Usage">Sample Usage</h2><pre><code class="js">#!<span class="regexp">/usr/</span>bin/env node

<span class="keyword">const</span> flaggedRespawn = <span class="built_in">require</span>(<span class="string">'flagged-respawn'</span>);

<span class="comment">// get a list of all possible v8 flags for the running version of node</span>
<span class="keyword">const</span> v8flags = <span class="built_in">require</span>(<span class="string">'v8flags'</span>).fetch();

flaggedRespawn(v8flags, process.argv, <span class="function"><span class="keyword">function</span> (<span class="params">ready, child</span>) </span>{
  <span class="keyword">if</span> (ready) {
    <span class="built_in">console</span>.log(<span class="string">'Running!'</span>);
    <span class="comment">// your cli code here</span>
  } <span class="keyword">else</span> {
    <span class="built_in">console</span>.log(<span class="string">'Special flags found, respawning.'</span>);
  }
  <span class="keyword">if</span> (process.pid !== child.pid) {
    <span class="built_in">console</span>.log(<span class="string">'Respawned to PID:'</span>, child.pid);
  }
});
</code></pre>
<h2 id="Release_History">Release History</h2><ul>
