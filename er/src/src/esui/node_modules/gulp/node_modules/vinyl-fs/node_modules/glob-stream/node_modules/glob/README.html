...
<li><code>realpathCache</code> An optional object which is passed to <code>fs.realpath</code><br>to minimize unnecessary syscalls.  It is stored on the instantiated<br>Glob object, and may be re-used.</li>
</ul>
...
<li><code>nodir</code> Do not match directories, only files.  (Note: to match<br><em>only</em> directories, simply put a <code>/</code> at the end of the pattern.)</li>
<li><code>ignore</code> Add a pattern or an array of patterns to exclude matches.</li>
<li><code>follow</code> Follow symlinked directories when expanding <code>**</code> patterns.<br>Note that this can result in a lot of duplicate references in the<br>presence of cyclic links.</li>
<li><code>realpath</code> Set to true to call <code>fs.realpath</code> on all of the results.<br>In the case of a symlink that cannot be resolved, the full absolute<br>path to the matched entry is returned (though it will usually be a<br>broken symlink)</li>
</ul>
