...
<h2 id="Examples">Examples</h2><h3 id="Consuming_a_source_map">Consuming a source map</h3><pre><code><span class="keyword">var</span> rawSourceMap = {
...
</code></pre><h3 id="Generating_a_source_map">Generating a source map</h3><p>In depth guide:<br><a href="https://hacks.mozilla.org/2013/05/compiling-to-javascript-and-debugging-with-source-maps/"><strong>Compiling to JavaScript, and Debugging with Source Maps</strong></a></p>
<h4 id="With_SourceNode_(high_level_API)">With SourceNode (high level API)</h4><pre><code>function <span class="keyword">compile</span>(ast) {
  ...
  <span class="keyword">case</span> <span class="string">'BinaryExpression'</span>:
    <span class="keyword">return</span> <span class="keyword">new</span> SourceNode(
      ast.location.line,
      ast.location.column,
      ast.location.<span class="keyword">source</span>,
      [<span class="keyword">compile</span>(ast.left), <span class="string">" + "</span>, <span class="keyword">compile</span>(ast.right)]
    );
  ...
  <span class="keyword">default</span>:
    <span class="keyword">throw</span> <span class="keyword">new</span> Error(<span class="string">"Bad AST"</span>);
  }
}

var ast = parse(<span class="string">"40 + 2"</span>, <span class="string">"add.js"</span>);
console.log(<span class="keyword">compile</span>(ast).toStringWithSourceMap({
  <span class="keyword">file</span>: <span class="string">'add.js'</span>
}));
<span class="comment">// { code: '40 + 2',</span>
<span class="comment">//   map: [object SourceMapGenerator] }</span>
</code></pre><h4 id="With_SourceMapGenerator_(low_level_API)">With SourceMapGenerator (low level API)</h4><pre><code>var <span class="variable">map =</span> new SourceMapGenerator({
  file: <span class="string">"source-mapped.js"</span>
});

<span class="built_in">map</span>.addMapping({
  generated: {
...
console.log(<span class="built_in">map</span>.<span class="built_in">toString</span>());
// '{<span class="string">"version"</span>:<span class="number">3</span>,<span class="string">"file"</span>:<span class="string">"source-mapped.js"</span>,<span class="string">"sources"</span>:[<span class="string">"foo.js"</span>],<span class="string">"names"</span>:[<span class="string">"christopher"</span>],<span class="string">"mappings"</span>:<span class="string">";;;;;;;;;mCAgCEA"</span>}'
</code></pre><h2 id="API">API</h2><p>Get a reference to the module:</p>
<pre><code>// NodeJS
...
// <span class="keyword">Inside</span> Firefox
let sourceMap = {};
Components.utils.import('resource:///modules/devtools/SourceMap.jsm', sourceMap);
</code></pre><h3 id="SourceMapConsumer">SourceMapConsumer</h3><p>A SourceMapConsumer instance represents a parsed source map which we can query<br>for information about the original file positions by giving it a file position<br>in the generated source.</p>
...
<h4 id="SourceMapConsumer-prototype-sourceContentFor(source)">SourceMapConsumer.prototype.sourceContentFor(source)</h4><p>Returns the original source content for the source provided. The only<br>argument is the URL of the original source file.</p>
...
<h4 id="SourceNode-fromStringWithSourceMap(code,_sourceMapConsumer)">SourceNode.fromStringWithSourceMap(code, sourceMapConsumer)</h4><p>Creates a SourceNode from generated code and a SourceMapConsumer.</p>
<ul>
...
<h3 id="SourceNode-prototype-toStringWithSourceMap([startOfSourceMap])">SourceNode.prototype.toStringWithSourceMap([startOfSourceMap])</h3><p>Returns the string representation of this tree of source nodes, plus a<br>SourceMapGenerator which contains all the mappings between the generated and<br>original sources.</p>
...
<pre><code>exports[<span class="string">"test doing the foo bar"</span>] = <span class="function"><span class="keyword">function</span> <span class="params">(assert, util)</span> </span>{
  ...
};
